...
class HealthOrgSettings:
    weight_project_health: float = 0.60
    weight_touchpoints: float = 0.0  # disabled — reserved for future use
...
def health_settings_from_dict(settings: dict | None) -> HealthOrgSettings:
    raw = settings or {}
    touchpoints = float(raw.get("health_weight_touchpoints", 0.0))
    project = float(raw.get("health_weight_project_health", 0.60))
    # Legacy orgs may still store a touchpoint weight — fold it into project health.
    if touchpoints > 0:
        project += touchpoints
        touchpoints = 0.0
    return HealthOrgSettings(
        weight_project_health=project,
        weight_touchpoints=touchpoints,
